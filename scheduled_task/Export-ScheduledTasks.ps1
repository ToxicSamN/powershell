...
#
# Modified by sammy Shuck for nordstrom purposes
...
begin {
  ...
  If([string]::IsNullOrEmpty($ExportPath)){
  	...
  	$ExportPath = "\\nord\dr\Software\VMware\Reports\scripty_schtasks\a0319p184_schtasks_$($dateTime).csv"
	$LogFilePath = "\\nord\dr\Software\VMware\Reports\scripty_schtasks"
  }
  ...
  Function Rotate-Logs(){
	...
	[array]$allLogs = @()
	#get all files with the prefix of PrefixSearch
	$logFile = Get-ChildItem $LogFilePath
	[array]$allLogs = Get-ChildItem "$($logFile.Directory)\$($prefixSearch)*" | Sort -Descending LastWriteTime
  ...
  function get-scheduledtask2($computerName) {
    $ExcludedTasks = @("\Google","\{","\Microsoft\")
      ...
      foreach ($name in $TaskName) {
		...
		If(-not $SKIP){
	        ...
	        foreach ($action in $taskDefinition.Actions) {
			  ...
			  # PROPERTY: XML
	          $taskXml = $task.Xml
	          $output | add-member NoteProperty Xml ($taskXml | Out-String)
	          # Output the object
	          $output
	        }
		}
...
process {
  if ($PIPELINEINPUT) {
    get-scheduledtask2 $_ | Export-Csv $ExportPath -NoTypeInformation
  }
  else {
...
End{
	Rotate-Logs -LogFilePath $LogFilePath -prefixSearch "a0319p184_schtasks" -NumKeep 60
}
